...
# Probeer de autorefresh library te laden, anders gebruiken we een backup methode
try:
...
# ==========================================
# SOLAR PIEK PRO v14.2 - STABILITY FIX
# ==========================================
...
def fetch_mppt_data():
    try:
        r = requests.get(f"{URL_SYMO_BASE}&DataCollection=CommonInverterData", timeout=2.5).json()
        d = r['Body']['Data']
...
st.caption(f"🔄 Laatste update: {datetime.now(tz).strftime('%H:%M:%S')} | Device: {dot_s}{dot_g}")

c1, c2 = st.columns(2)
c1.metric("⚡ Oogst vandaag", f"{oogst_vandaag:.1f} kWh")
c2.metric("🏆 All-time Piek", f"{max(all_time_peak_sheet, st.session_state.p_total_peak):,.0f} W")
...
# Auto-refresh onderaan plaatsen voor stabiliteit
if st_autorefresh:
