...
# Probeer startwaarde en piek te laden uit Sheet
try:
    ...
    if not vandaag.empty:
        ...
        if start_val is not None:
            st.session_state.start_kwh_dag = float(start_val)
...
# Start kWh vastleggen (belangrijk!)
...
temp, desc, hum, weather_icon = get_weather()   # (gebruik je vorige weer functie)
...
if st.button("🔄 Reset Oogst vandaag (test)", type="secondary"):
    st.session_state.start_kwh_dag = None
