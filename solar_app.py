...
# ==========================================
# SOLAR PIEK PRO v14.5 - KWH METER ONLY FIX
# ==========================================
...
# We gebruiken poort 8080 en 8081 voor de twee kWh meters
...
def fetch_hw_data(url):
    try:
        ...
        # kWh meters gebruiken 'total_power_import_kwh' of export afhankelijk van aansluiting
        kwh = float(r.get('total_power_import_kwh') or r.get('total_power_export_kwh') or 0)
...
if st.session_state.start_kwh_dag is None or st.session_state.start_kwh_dag == 0:
    st.session_state.start_kwh_dag = stand_gisteren if (stand_gisteren and stand_gisteren > 0) else kwh_nu
...
if st.session_state.start_kwh_dag and kwh_nu > 0:
...
st.caption(f"🔄 Laatste sync: **{st.session_state.last_sync_time}** | Status: {dot_s}{dot_g}")
