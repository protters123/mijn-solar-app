...
# ==========================================
# SOLAR PIEK PRO - HERSTELDE WERKENDE VERSIE
# ==========================================
...
# Gebruik de poorten die in je router open staan
...
def fetch_hw_data(url):
    try:
        ...
        # Verbeterde kWh uitlezing voor HomeWizard Meters
        # Hij kijkt naar export (zonnestroom) of import velden
        wh = float(r.get('total_power_import_t1_kwh') or r.get('total_power_export_kwh') or 0)
        return power, wh, "🟢"
    except:
        return 0.0, 0.0, "🔴"
...
st.write(f"⏰ Laatste update: {nu_lokaal.strftime('%H:%M:%S')} ({vandaag_nl})")

col_a, col_b = st.columns(2)
with col_a:
    st.metric("⚡ Totaal Live", f"{val_t:,.0f} W")
with col_b:
    # Oogst berekening (totaal stand)
    st.metric("🍯 Totaal kWh", f"{kwh_s + kwh_g:,.2f}")
...
with c2:
    st.markdown("### 📊 Totaal")
    st.metric("Piek Vandaag", f"{st.session_state.p_symo_peak + st.session_state.p_galvo_peak:,.0f} W")
with c3:
